...
async def reset(req: ResetRequest = ResetRequest()):
    if not HAS_ENV:
        ...
        return {"observation":{"task_id":t["task_id"],"task_name":t["name"],"difficulty":t["difficulty"],"max_steps":t["max_steps"],"schema_ddl":t.get("schema_ddl",""),"business_question":t.get("business_question",""),"hint":t.get("hint",""),"step":0,"previous_error":None},"reward":0.0,"done":False,"info":{}}
...
async def step(req: StepRequest):
    if not HAS_ENV:
        return {"observation":{},"reward":0.0,"done":False,"info":{"feedback":"Backend env not loaded - cannot score in demo mode."}}
